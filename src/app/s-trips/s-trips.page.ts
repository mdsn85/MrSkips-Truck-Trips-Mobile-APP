...
export class STripsPage extends RouterPage implements OnDestroy {
  ...
  TripSelected(Id, $event) {
    console.log("selecting");
    if ($event.target.checked) {
      this.selectedTrips.push(Id);
    } else {
      var index = this.selectedTrips.indexOf(Id);
      this.selectedTrips.splice(index, 1);
    }

    console.log("selected trip length=" + this.selectedTrips.length);
  }

  async SignSelected() {
    console.log("Enter SignSelected value = " +this.selectedTrips);
    if (this.selectedTrips != null && this.selectedTrips.length > 0) {
      

      let selectedTripsStringify = JSON.stringify(this.selectedTrips);
      console.log("selected trip =" + selectedTripsStringify);
      const alert = await this.alertCtrl.create({
        header: 'Confirm sign selected trips',
        message: 'Are you sure you want to sign selected trip?',
        buttons: [
          {
            text: 'No',
          ...
          },
          {
            ...
            handler: () => {
              ...
              // routerLink="/sign/{{ trip.Id }}"
              this.navController.navigateForward('/sign/'+selectedTripsStringify)
      ...
      const alert = await this.alertCtrl.create({
        header: 'There is no trip selected',
        message: 'Please select trips and try again',
        buttons: [
          {
            text: 'OK',
            role: 'cancel',
